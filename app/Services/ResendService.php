...
HTML;
     ...
     */
    public function enviarCodigoRecuperacion(string $correo, string $codigo, string $nombre): bool
    {
        try {
            $html = $this->getEmailRecuperacionTemplate($codigo, $nombre);

            Mail::html($html, function ($message) use ($correo) {
                $message->from($this->fromEmail, $this->fromName)
                    ->to($correo)
                    ->subject("Recuperación de contraseña - {$this->fromName}");
            });
     ...
     */
    protected function getEmailRecuperacionTemplate(string $codigo, string $nombre): string
    {
        $year = date('Y');
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Recuperación de contraseña — Octágono</title>
</head>
<body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f4f7fa;">
    <table role="presentation" style="width: 100%; border-collapse: collapse;">
        <tr>
            <td style="padding: 40px 0;">
                <table role="presentation" style="width: 100%; max-width: 620px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.08);">
                    ...
                    <!-- Banner -->
                    <tr>
                        <td style="background: linear-gradient(135deg, #fef3c7 0%, #fde68a 100%); padding: 20px 40px; text-align: center; border-bottom: 2px solid #f59e0b;">
                    ...
                    <tr>
                        <td style="padding: 40px 40px 24px 40px;">
                            ...
                            <p style="margin: 0 0 28px 0; color: #6b7280; font-size: 15px; line-height: 1.7;">
                                Recibimos una solicitud para restablecer la contraseña de tu cuenta en Octágono.
                                Usa el siguiente código para continuar con el proceso:
                            </p>

                            <!-- Code Box -->
                            <div style="background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%); border: 2px solid #0ea5e9; border-radius: 12px; padding: 28px; text-align: center; margin-bottom: 28px;">
                                <p style="margin: 0 0 8px 0; color: #0369a1; font-size: 12px; text-transform: uppercase; letter-spacing: 1px; font-weight: 600;">Tu código de recuperación</p>
                                <p style="margin: 0; color: #0c4a6e; font-size: 48px; font-weight: 700; letter-spacing: 12px; font-family: 'Courier New', monospace;">{$codigo}</p>
                            </div>
                            ...
                            <div style="background-color: #f0f9ff; border: 1px solid #bae6fd; border-radius: 10px; padding: 16px 20px;">
                                <p style="margin: 0; color: #0369a1; font-size: 13px; line-height: 1.6;">
                                    <strong>🔒 Seguridad:</strong> Nunca compartas este código con nadie. El equipo de Octágono nunca te pedirá tu código de recuperación.
                                </p>
                            </div>
                        </td>
                    </tr>
