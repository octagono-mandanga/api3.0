...
class ConfiguracionInicialController extends Controller
{
     ...
     * POST /api/instituciones/{solicitudId}/configuracion-inicial
     *
     * El parámetro {solicitudId} es el UUID de la solicitud de activación.
     * En esta llamada se crea la institución si aún no existe.
     */
    public function store(Request $request, string $solicitudId): JsonResponse
    {
        try {
            ...
            if (!$solicitud) {
                return response()->json([
                    'status'  => 'error',
                    'message' => 'Enlace de configuración no válido.',
                ], 404);
            }
            ...
            if ($solicitud->estado !== 'completada') {
                return response()->json([
                    'status'  => 'error',
                    'message' => 'La solicitud no ha sido completada. Finalice primero la verificación.',
                ], 403);
            }

            // 2. Buscar o crear la institución vinculada a esta solicitud
            if ($solicitud->institucion_id) {
                // Ya existe — buscar la institución en la tabla
                $institucion = Institucion::find($solicitud->institucion_id);
                if (!$institucion) {
                    return response()->json([
                        'status'  => 'error',
                        'message' => 'Error interno: institución vinculada no encontrada.',
                    ], 500);
                }
            } else {
                // Primera llamada: crear la institución con datos básicos de la solicitud
                $institucion = Institucion::create([
                    'nombre_legal'  => $solicitud->nombre_institucion,
                    'nombre_corto'  => $solicitud->nombre_institucion,
                    'email_oficial' => $solicitud->correo,
                    'telefono'      => $solicitud->telefono,
                    'estado'        => 'configuracion',
                ]);

                // Vincular la solicitud con la institución creada
            ...
            // 3. Validar datos de configuración del wizard
            $validated = $request->validate([
                ...
                'institucion.departamento'     => 'nullable|integer',
                'institucion.municipio'        => 'nullable|integer',
                ...
                'sede_principal.municipio_id'  => 'nullable|integer',
